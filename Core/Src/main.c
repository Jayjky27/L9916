...
#define OTP_WRITE_REGISTER      0x09
#define OTP_WRITE_COMMAND       0x0A
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  uint8_t ddata = 0;
  uint8_t *data_p = &ddata;
  HAL_StatusTypeDef L9916_Read_OTP(data_p);
  __asm__("BKPT");
...
HAL_StatusTypeDef L9916_Write_OTP(uint8_t row, uint8_t column, uint8_t data)
{
    ...
    write_command[0] = OTP_WRITE_REGISTER;
    ...
    write_command[0] = OTP_WRITE_COMMAND;
...
void Entering_program_mode(void)
{
    ...
    // Apply 15.7V to battery terminal (not shown here, ensure it's connected externally)

    // Apply 0xAFF3 bit pattern to IGN pin synchronously with PH clock
    // Example for software emulation:
    for (int i = 0; i < 16; i++)
    {
        if ((0xAFF3 >> (15 - i)) & 0x1)
        {
